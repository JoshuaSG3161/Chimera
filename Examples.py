...
class WheelController:
    ...
    def __init__(self):
        self.wheel_pub = rospy.Publisher(
            "/gazebo_wheelControl/wheelSpeedTopic", wheelSpeed, queue_size=1)
...
class LaserListener:
    ...
    def __init__(self):
        self.laserSub = rospy.Subscriber(
            "/leddar/leddarData", LaserScan, self.laser_callback, queue_size=1)
        self.laserRanges = [1, 1, 1, 1, 1, 1,
...
class LocationHeading:
    ...
    def __init__(self):
        self.fixSub = rospy.Subscriber(
            "/fix/metres", Point, self.fix_callback, queue_size=1)
        self.headingSub = rospy.Subscriber(
            "/heading", Float32, self.heading_callback, queue_size=1)
        self.x = 0.0
        self.y = 0.0
        self.z = 0.0
        self.heading = 0.0
...
while not rospy.is_shutdown():  # this will run until gazebo is shut down or CTRL+C is pressed in the ubuntu window that is running this code
    # minRange = 999 #initialize minRange to a value larger than what will be recieved
    # for x in range(len(laser.laserRanges)): #iterate through the ranges list (the len function gives the length of the list)
    ...
    # Instead of iterating through every value, you can just use the build-in min function
    minRange = min(laser.laserRanges)
    ...
    print("Heading: ", locHead.heading, "x val: ", round(locHead.x, 2), "y val: ", round(locHead.y, 2), "z val: ",
          round(locHead.z, 2),  "MinLaser: ", round(min(laser.laserRanges), 2))  # print data to the command line
